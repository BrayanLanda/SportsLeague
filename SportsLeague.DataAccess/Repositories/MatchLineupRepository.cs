    ...
    public class MatchLineupRepository : GenericRepository<MatchLineup>, IMatchLineupRepository
    {
        ...
        public async Task<int> CountStartersByMatchAndTeamAsync(int matchId, int teamId)
        {
            return await _dbSet.CountAsync(
                ml => ml.MatchId == matchId && ml.IsStarter && ml.Player.TeamId == teamId);
        }

        public async Task<MatchLineup?> GetByMatchAndIdAsync(int matchId, int id)
        {
            return await _dbSet
                .Where(ml => ml.MatchId == matchId && ml.Id == id)
                .Include(ml => ml.Player)
                    .ThenInclude(p => p.Team)
                .FirstOrDefaultAsync();
        }
